...
void
BasicSimplifier::breakNodePair(Graph & target_graph, int node_id) {
...
void
BasicSimplifier::breakOneDegreeNode(Graph & target_graph, int node_id) {
...
void
BasicSimplifier::breakZeroDegreeNode(Graph & target_graph, int node_id) {
...
bool
BasicSimplifier::canPair(int n1, int n2, const node_tertiary_data_s & td1, const node_tertiary_data_s & td2) const {
	  ...
	  if (create_groups) {
		...
		if (td1.indegree == 0 && td1.outdegree == 0) {
		  ...
		  if (node_pairs.count(np.second)) {
		    cerr << "BasicSimplifier: error with pairs!\n";
		  } else {
